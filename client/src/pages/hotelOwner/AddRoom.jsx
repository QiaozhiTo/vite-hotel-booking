...
const AddRoom = () => {
    //axios to make the API call, getToken for the authorization token
            ...
            <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
                {Object.keys(images).map((key) =>(
                    <label htmlFor= {`roomImage${key}`} key={key} >
                        ...
                        className='max-h-13 cursor-pointer opacity-80'></img>
                        <input type="file" accept='image/*' id={`roomImage${key}`} hidden
                        onChange={e=>setImages({...images, [key]:e.target.files[0]})}></input>
                    </label>
